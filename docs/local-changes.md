...
2026-09-15 定向补录：内置媒体查看器双击进入视频内容全屏后，`Ctrl+C` 无法复制当前帧；已改为全屏分支不再吞掉 Ctrl / Alt 组合键。新增 `Ctrl+R` 旋转 90°、`Alt+R` 水平镜像翻转。双击进出全屏时，单击松开不再立刻切换播放/暂停，避免播放中短暂停顿、暂停时进度被推走。第 7.1、7.3 节同步更正，并新增第 7.6 节列出查看器全部快捷操作。补录针对当前工作目录中的按键逻辑，未编译或运行客户端。
...
固定回归：单图/相册/GIF/视频预览；OpenGL/Raster；不同界面缩放；滚轮四区域、OSD、重开重置、视频双击（播放/暂停状态与暂停进度保持不变）、数字键/长按方向键/PageUp；视频内容全屏后的 `Ctrl+C` 复制帧、`Ctrl+R` 旋转和 `Alt+R` 镜像。
...
画面单击松开后，播放/暂停推迟到系统双击间隔结束才执行；若在间隔内形成双击，则取消这次切换，只进出视频内容全屏。原先第一次松开已经 `playbackPauseResume()`、双击再切回去，播放中会短暂停顿，暂停时会先播出几帧把进度推走。单击暂停因此会略晚于松开，延迟不超过系统双击间隔。
...
| --- | --- |
| `Space` 短按 | 播放 / 暂停 |
| 画面单击 | 播放 / 暂停（松开后等待系统双击间隔；双击进出全屏时不切换） |
...
回归重点：窗口非全屏与视频内容全屏下的 `Ctrl+C` / `Ctrl+S` / 缩放 / `Ctrl+R` / `Alt+R`；`Esc` 退出全屏而不关窗口；播放中与暂停中双击进出全屏时播放状态和暂停进度不变；单击暂停略有延迟；暂停逐帧；章节跳转；`1`–`3` 改倍速与全屏 `4`–`9` 跳转互不覆盖；长按空格 2.5× 与长按方向键 0.5× / 2×；滚轮增强开 / 关。
