...
FormatPointer MakeFormatPointer(
	...
	io->seekable = ((seek != nullptr) && settings.seekableOnOpen);
	if (!settings.seekableOnOpen) {
		io->seek = nullptr;
	}
	...
	av_dict_set(&options, "usetoc", settings.seekableOnOpen ? "1" : "0", 0);
	...
	if (!settings.seekableOnOpen && result->pb) {
		result->pb->seek = seek;
