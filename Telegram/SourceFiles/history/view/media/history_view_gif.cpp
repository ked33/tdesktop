...
void Gif::drawCornerStatus(
	...
	const auto text = isVideo
		? (durationText.isEmpty()
			...
			: (durationText + ' ' + dcText))
		: baseText;
	const auto download = allowDownload
		&& downloadInCorner()
		&& !dataLoaded()
	...
	PaintVideoCornerStatus(p, context, {
		...
		.mute = (_streamed && isVideo && !download),
		.showSecondLine = isVideo,
	});
}
...
TextState Gif::getStateGrouped(
	...
	if (_data->isVideoFile() || fullFeatured) {
