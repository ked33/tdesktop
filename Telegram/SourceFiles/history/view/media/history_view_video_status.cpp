...
void PaintVideoCornerStatus(
	...
	const auto secondLine = status.download || status.showSecondLine;
	...
	p.drawTextLeft(
		...
		textWidth);
	if (secondLine) {
		const auto downloadTextTop = status.download
			? (statusY + font->height + (2 * freeHeight / 3) - padding.y())
			: (statusTextTop + font->height);
		p.drawTextLeft(
			statusX + addLeft,
			downloadTextTop,
			outerWidth,
