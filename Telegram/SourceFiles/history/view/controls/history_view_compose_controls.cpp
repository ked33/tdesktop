...
bool ComposeControls::requestEditableMessageNavigation(QKeyEvent *keyEvent) {
	if (!_history
...
void ComposeControls::initKeyHandler() {
	_wrap->events(
		...
		if (requestEditableMessageNavigation(keyEvent)) {
			return;
		}
		if (!hasModifiers
			&& ((key == Qt::Key_Up)
	...
	base::install_event_filter(_wrap.get(), _field->rawTextEdit(), [=](
		...
		if (requestEditableMessageNavigation(k)) {
