...
SelectedItems ListWidget::collectSelectedItems(
		const SelectedMap &selected) const {
	...
	if (!selected.empty()) {
		...
		if (usesGlobalSelectedMessages()) {
			...
			for (const auto &id : session().data().globalSelectedMessages()) {
				const auto i = remaining.find(id);
				if (i != remaining.end()) {
					items.push_back(transformation(*i));
					remaining.erase(i);
				}
			}
			for (const auto &item : remaining) {
				items.push_back(transformation(item));
			}
		} else {
			std::transform(
				selected.begin(),
...
std::vector<not_null<HistoryItem*>> ListWidget::selectionUpTo(
		not_null<HistoryItem*> item) const {
	const auto toView = viewForItem(item);
	if (_selected.empty()
		|| !toView
		|| !_delegate->listIsItemGoodForSelection(item)) {
		return {};
	...
	auto nearest = end(_items);
	auto distance = int(_items.size());
	for (auto i = begin(_items); i != end(_items); ++i) {
		const auto selected = (*i)->data();
		if (!_selected.contains(selected->fullId())
			|| !_delegate->listIsItemGoodForSelection(selected)) {
			continue;
		}
		const auto diff = std::abs(int(i - toIt));
		if (diff < distance) {
			nearest = i;
			distance = diff;
		}
	}
	if (nearest == end(_items) || nearest == toIt) {
		return {};
	}
	const auto from = (nearest < toIt) ? nearest : toIt;
	const auto to = (nearest < toIt) ? toIt : nearest;
	const auto result = collectBetween((*from)->data(), (*to)->data());
