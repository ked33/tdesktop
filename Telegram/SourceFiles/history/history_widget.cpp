...
bool HistoryWidget::editableMessageNavigationRequest(QKeyEvent *e) {
	...
	const auto currentId = (requestedCurrentId.peer == _history->peer->id)
		...
		: FullMsgId();
	if (edit && !currentId) {
		auto last = _history->lastMessage();
		while (last && last->isSponsored()) {
			const auto view = last->mainView();
			const auto previous = view ? view->previousDisplayedInBlocks() : nullptr;
			last = previous ? previous->data().get() : nullptr;
		}
