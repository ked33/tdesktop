...
[[nodiscard]] bool WaitForMp4LayoutForSeek(
	...
	while (entry->mp4Layout.load() == 0) {
		if ((crl::now() - started) >= kMp4LayoutWaitTimeout) {
			MPV_STREAMING_LOG(("MPV Streaming (Special): MP4 layout wait timed out for token %1 offset=%2 activeRequests=%3.")
				.arg(token)
				.arg(offset)
		...
		void handleConnection(qintptr descriptor) {
			...
			if (request.indexRevision && !seekPatches) {
				if (!SendResponse(socket, "404 Not Found", {
					...
					MPV_STREAMING_LOG(("MPV Streaming (Special): Expired index response disconnected."));
				}
				return;
			}
...
[[nodiscard]] bool StartManagedPlayer(
	...
	Mpv::ManageIndexReload(raw, Mpv::ControlIndex(launch.index), launch.duration);
