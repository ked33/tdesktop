...
MediaViewerBrightnessBox::MediaViewerBrightnessBox(QWidget *parent) {
}

void MediaViewerBrightnessBox::prepare() {
	setTitle(tr::lng_settings_media_viewer_brightness_title());
	...
	const auto current = GetEnhancedInt("media_viewer_brightness");
	const auto initial = (current >= 10 && current <= 100)
		? ((current / 10) * 10)
		: 100;
	_brightnessGroup = std::make_shared<Ui::RadiobuttonGroup>(initial);
...
QString MediaViewerBrightnessBox::BrightnessLabel(int percent) {
	return tr::lng_settings_preview_brightness_percent(
		tr::now,
		lt_percent,
		QString::number(percent));
...
void MediaViewerBrightnessBox::save() {
	SetEnhancedValue("media_viewer_brightness", _brightnessGroup->current());
	EnhancedSettings::Write();
	closeBox();
}
